<template>
  ...
  // Fetch the post tagged with "schnuppern"
  function fetch_schnuppern_post() {
    loading.value = true;
    wp.get_posts(undefined, ['schnuppern'])
      .then((response: Post[]) => {
        if (response.length > 0) {
          ...
          useHead({
            ...
            meta: [
              { name: 'description', content: post.value.excerpt.replace(/<[^>]+>/g, '') || 'Schnuppern - HTL Weiz' }
            ]
          });
        }
      })
  ...
  // Initial fetch of the "Schnuppern" post
  fetch_schnuppern_post();
  </script>