<template>
  ...
  // Fetch the post tagged with "kennenlernangebote"
  function fetch_kennenlernangebote_post() {
    loading.value = true;
    wp.get_posts(undefined, ['kennenlernangebote'])
      .then((response: Post[]) => {
        if (response.length > 0) {
          ...
          useHead({
            ...
            meta: [
              { name: 'description', content: post.value.excerpt.replace(/<[^>]+>/g, '') || 'Kennenlernangebote - HTL Weiz' }
            ]
          });
        }
      })
  ...
  // Initial fetch of the "Kennenlernangebote" post
  fetch_kennenlernangebote_post();
  </script>