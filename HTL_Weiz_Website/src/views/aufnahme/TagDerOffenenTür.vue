...
import HeaderBanner from '@/components/HeaderBanner.vue';
  ...
  // Fetch the post tagged with "tdot"
  function fetch_tdot_post() {
    loading.value = true;
    wp.get_posts(undefined, ['tdot'])
      .then((response: Post[]) => {
        if (response.length > 0) {
          ...
          useHead({
            ...
            meta: [
              { name: 'description', content: post.value.excerpt.replace(/<[^>]+>/g, '') || 'Tag der offenen Tür - HTL Weiz' }
            ]
          });
        }
      })
  ...
  // Initial fetch of the "Tag der offenen Tür" post
  fetch_tdot_post();
  </script>