<template>
  ...
  // Fetch the post tagged with "vorteilehtl"
  function fetch_vorteilehtl_post() {
    loading.value = true;
    wp.get_posts(undefined, ['vorteilehtl'])
      .then((response: Post[]) => {
        if (response.length > 0) {
          ...
          useHead({
            ...
            meta: [
              { name: 'description', content: post.value.excerpt.replace(/<[^>]+>/g, '') || 'Vorteile einer HTL - HTL Weiz' }
            ]
          });
        }
      })
  ...
  // Initial fetch of the "Vorteile HTL" post
  fetch_vorteilehtl_post();
  </script>